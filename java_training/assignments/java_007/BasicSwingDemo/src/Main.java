...
public class Main {
	...
	interface SetFrameStuff {
		JFrame run(JFrame frm, int width, int height, int onExit, String label);
	}

	static SetFrameStuff setFrameStuff = (JFrame frm, int width, int height, int onExit, String label) -> {
		...
		*/
		...
		SwingUtilities.invokeLater(new Runnable() {
			...
			public void run() {
				...
				frm = demo.getFrame();
				frm = setFrameStuff.run(frm, 375, 100, JFrame.EXIT_ON_CLOSE, "My label");
				frm.setVisible(true);
			}
		});
	}
